...
class Report:
    ...
    def write_config(self, config_path: str="") -> None:
        ...
        self.fid.write(
            "# Configuration of {}\n\n__Located at {}__\n".format(self.description, ", ".join(config_path))
        )
        self.fid.write("```\n")
        #self.fid.write(str(config.tech.as_str(key_width=25, width=70, only_used=True)))  #TODO: Write configuration given in config_path.
        self.fid.write("\n```\n")
        self.fid.write("\\newpage\n\n")
    ...
    def title_page(self):
        ...
        #user_info = util.get_user_info() #TODO: transfer functionality from Where to Midgard

        self.fid.write(
            """---
title: {title}
author: Midgard v{version} [{user}]
date: {nowdate:%Y-%m-%d}
---
    """.format(
                title=title,
                version=midgard.__version__,
