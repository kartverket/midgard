...
class SinexSiteParser(SinexParser):
    ...
    # WRITE DATA
    #
    def as_site_info(self) -> "SiteInfo":
        """Store site related information in site_info module data structure

        Returns:
            Site dictionary, whereby keys are the site identifiers and values are a site_info module objects related 
            to keys 'antenna', 'receiver' and 'site_coord'. The site dictionary has following structure:

                      self.site[site] = { 'antenna':     AntennaHistorySinex object,
                                          'receiver':    ReceiverHistorySinex object,
                                          'site_coord':  SiteCoordHistorySinex object,
                      }

            whereby the AntennaHistoySinex, ReceiverHistorySinex and SiteCoordHistorySinex objects has following 
            attributes:

        |  Attribute           | Type         | Description                                                           |
        |----------------------|--------------|-----------------------------------------------------------------------|
        | date_from            | datetime     | Installing date of an antenna                                         |
        | date_to              | datetime     | Removing date of an antenna                                           |
        | history              | dict         | Dictionary with antenna history information                           |
        | source               | str          | Source of data e.g. "sinex"                                           |
        | source_path          | PosixPath    | File path of SINEX file                                               |
        ...
        |----------------------|-------------------|------------------------------------------------------------------|
        | get                  | date              | Get antenna information for a given date                         |

        """
        site_info = dict()

        # Save data in site_info module data structure
        for sta in self.data.keys():

            site_info.setdefault(sta, dict())
            site_info[sta]["antenna"] = Antenna.get_history(
                source="sinex", 
                station=sta, 
            ...
            site_info[sta]["receiver"] = Receiver.get_history(
                source="sinex", 
                station=sta, 
                source_path=self.file_path,
                source_data=self.data,
            ...
            site_info[sta]["site_coord"] = SiteCoord.get_history(
                source="sinex", 
                station=sta, 
                source_path=self.file_path,
                source_data=self.data,
