...
DatasetField = namedtuple(
    "DatasetField", ["name", "category", "dtype"]
)
    ...
    def as_dataset(self) -> "Dataset":
       ...
       The fields above are given for 'apriori', 'value' and 'sigma' Dataset collections.
        ...
        dset.add_time(
                "time", 
                val=Time(
                        (self.data["time_past_j2000"] + 13.0) * Unit.second2day + 2451545.0,  
                        scale="tt", 
                        fmt="jd"
                ).gps
        )
        ...
        for name in set(self.data["name"]):
            ...
            if parameter in field.keys():
                ...
                for collection in collections:
                    ...
                    log.debug(f"Add dataset field '{field_name}' for parameter '{parameter}' and identifier '{identifier}'.")
                    ...
                    if field[parameter].dtype == "float": 
                        ...
                        if parameter == "Clk Bias":
                            field_name = f"{collection}.satellite_clock" if len(identifier) == 5 else f"{collection}.receiver_clock"
                    ...
                    # Add position fields to Dataset
                        ...
                        if field_name not in dset.fields:
                            dset.add_position(
                                    ...
                                    time=dset.time, 
                                    system="trs",
                                    val= np.full((dset.num_obs, 3), np.NaN),
                            )
                    ...
                    # Add posvel fields to Dataset
                        ...
                        if field_name not in dset.fields:
                            dset.add_posvel(
                                    ...
                                    time=dset.time, 
                                    system="trs",
                                    val= np.full((dset.num_obs, 6), np.NaN),
                            )
